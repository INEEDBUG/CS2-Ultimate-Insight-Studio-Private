...
- Installed `2.5.14-rc.42` window acceptance: the League lab opens at its header instead of retaining a stale deep scroll position; the disconnected Mini renders its bootstrap/context/settings surface rather than a blank page; its account-write button remains disabled; the Mini close control removes only the auxiliary window and leaves the main process running. The installed settings were read back with every known manual/automatic account-write flag disabled.
