...
- **Steam 官方比赛同步** — 自动识别本机当前 Steam 账号，并使用 Valve 当前支持的 `GetNextMatchSharingCode` 流程，以一次性配置的 API Key、SteamID64、游戏认证码和一场已知分享码增量同步官匹记录；可一键建立全部新 Demo 的后台下载任务。下载时只需 Steam 客户端保持登录，无需每次启动 CS2；完整地图、比分和玩家评价仍以下载后的本地 Demo 解析结果为准。同步结果缓存在本地，避免每次启动重复请求整段历史。Steam OpenID 只证明账号身份，不能代替游戏认证码或直接返回逐局数据。
...
> **Steam 凭据安全：** Steam Web API Key 与游戏认证码只保存在本机配置文件中，界面返回值会脱敏，HTTP 客户端日志不会记录包含凭据的查询串。Valve 的官方接口只提供分享码链；本项目不会把它伪装成未经 Demo 验证的完整 K/D 战绩。

> **Steam 登录与 csstats.gg 边界：** “使用 Steam 登录”通过 Steam 官方 OpenID 页面完成，只验证并绑定 SteamID，不会接触密码或复用浏览器 Cookie。OpenID 本身不返回 CS2 逐局战绩；自动同步仍需一次性配置 Valve 游戏认证码。csstats.gg 当前未提供供本项目使用的公开授权 API，且其条款禁止自动脚本与数据抓取，因此本项目不会爬取或嵌入其登录会话，而是在本机用 Valve 分享码和 Demo 实现同类自动同步与分析。
