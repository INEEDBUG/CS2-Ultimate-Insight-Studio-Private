...
- **英雄联盟实验室** — 与 CS2 功能共用同一个桌面客户端和托盘，按 LeagueAkari 的本机 LCU/SGP 方案兼容 Riot 与 WeGame/Tencent 客户端。支持自动接受、按模式/位置选择与禁用英雄、备战席换取/重随、按普通/排位分路/ARAM/URF 等场景配置符文与召唤师技能、自动点赞、自动匹配/返回房间/掉线重连、完整队列邀请策略、Riot ID 跨区玩家检索与目标区服 SGP 分页战绩、最近遇见、本地标签、100 场 SQLite 战绩收集、基础与 AND/OR 组合筛选及筛选预设、SGP 排位/战绩/藏品挑战、实时十人分析与组排推断、双方当前打野的首开/路线/早期抓人时间线画像、独立实时对局窗口，以及按阶段自动显示的置顶 Mini 面板、备战席/皮肤操作和可选复活倒计时。另提供主播隐私别名与原生窗口捕获保护、LeagueClientUx 尺寸修复与居中、可逆的游戏设置文件只读/可写切换、仅针对当前前台 `League of Legends.exe` 的双确认终止工具，以及只生成不自动发送的近期表现、组排关系和打野侦察草稿。LCU、Riot Client 与 SGP 令牌只保存在运行内存，不写入磁盘、不上传；所有会影响账号、客户端或对局的自动操作默认关闭。
