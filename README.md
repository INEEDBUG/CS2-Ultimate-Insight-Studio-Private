...
- **英雄联盟实验室** — 与 CS2 功能共用同一个桌面客户端和托盘，按 LeagueAkari 的本机 LCU 方案兼容 Riot 与 WeGame/Tencent 客户端。支持自动接受、按模式/位置选择与禁用英雄、备战席换取/重随、符文与召唤师技能配置、自动点赞、自动匹配/返回房间/掉线重连、按队列处理邀请、Riot ID 玩家检索与分页战绩、最近遇见、本地标签、实时十人分析与组排推断、独立实时对局窗口，以及按阶段自动显示的置顶 Mini 面板和可选复活倒计时。LCU 令牌只保存在运行内存，不写入磁盘、不上传；所有会影响账号或对局的自动操作默认关闭。
