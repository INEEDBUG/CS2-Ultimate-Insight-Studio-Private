...
- **League 游戏内预设** — 支持固定多行文字与可选全局快捷键，并按己方、敌方或双方生成近期表现、组排关系和打野画像。房间与选人阶段使用 LCU 对话；游戏中仅在 `League of Legends.exe` 仍处于前台时逐行发送。账号写入总开关、预设发送开关和每条快捷键均默认关闭，手动发送还需输入确认短语。
