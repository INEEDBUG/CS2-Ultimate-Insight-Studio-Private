...
- **英雄联盟实验室** — 与 CS2 功能共用同一个桌面客户端和托盘，按 LeagueAkari 的本机 LCU 方案兼容 Riot 与 WeGame/Tencent 客户端。支持自动接受、英雄选择/禁用、符文与召唤师技能配置、自动点赞、返回房间、掉线重连、邀请策略、最近战绩，以及可置顶的独立 Mini 面板。LCU 令牌只保存在运行内存，不写入磁盘、不上传；总开关关闭时不会执行写入操作。
