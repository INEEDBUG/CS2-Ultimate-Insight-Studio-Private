...
const CONFIG_KEYS = [
  ["default", "默认回退"], ["normal", "匹配 / 普通模式"], ["ranked-default", "排位默认"],
  ["ranked-top", "排位 · 上路"], ["ranked-jungle", "排位 · 打野"], ["ranked-middle", "排位 · 中路"],
  ["ranked-bottom", "排位 · 下路"], ["ranked-utility", "排位 · 辅助"], ["aram", "极地大乱斗"],
  ["urf", "无限火力"], ["nexusblitz", "极限闪击"], ["ultbook", "终极魔典"],
];
...
export default function LeagueChampionLoadoutEditor({ loadouts = [], onChange, onError }) {
  ...
  return <div className="mt-4 space-y-4">
    ...
    <div className="grid gap-2">{loadouts.map((item)=>{const champion=championsById.get(Number(item.champion_id));const configKey=item.config_key||"default";return <div key={`${item.champion_id}:${configKey}`} className="flex items-center gap-3 rounded-xl border border-cs2-border-subtle px-3 py-2 text-xs"><img src={getLeagueChampionIconUrl(item.champion_id)} alt="" className="h-9 w-9 rounded-lg bg-white/5 object-cover"/><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{champion?.name||`英雄 ${item.champion_id}`} · {CONFIG_KEYS.find(([id])=>id===configKey)?.[1]||configKey}</strong><span className="text-cs2-text-muted">{stylesById.get(Number(item.primary_style_id))?.name||item.primary_style_id} / {stylesById.get(Number(item.sub_style_id))?.name||item.sub_style_id} · {spellsById.get(Number(item.spell1_id))?.name||item.spell1_id} + {spellsById.get(Number(item.spell2_id))?.name||item.spell2_id}</span></span><button onClick={()=>onChange(loadouts.filter((value)=>!(Number(value.champion_id)===Number(item.champion_id)&&(value.config_key||"default")===configKey)))} aria-label="删除配置" className="rounded-lg p-2 text-red-300 hover:bg-red-400/10"><X className="h-4 w-4"/></button></div>;})}</div>
  </div>;
}
