...
import { AlertTriangle, Boxes, Gift, Power, RefreshCw, ScrollText, Users } from "lucide-react";
import { fetchLeagueToolkitOverview, sendLeagueChatMessage, terminateLeagueGameClient, updateLeagueChatPresence } from "../../api/leagueLabApi";
...
export default function LeagueToolkitOverview({ onError }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="rounded-2xl border border-cs2-border bg-cs2-bg-elevated p-4"><h3 className="text-sm font-bold">房间 / 英雄选择预设消息</h3><p className="mt-1 text-xs text-cs2-text-muted">每行一条，点击后合并发送到当前 LCU 对话；不会自动发送，游戏内键盘模拟暂未启用。</p><textarea value={preset} onChange={(event)=>setPreset(event.target.value)} maxLength={3000} rows={4} placeholder="例如：大家好，祝游戏愉快" className="mt-4 w-full resize-y rounded-xl border border-cs2-border bg-cs2-bg-input px-3 py-2.5 text-sm outline-none"/><div className="mt-2 flex justify-end"><button onClick={sendPreset} disabled={busy||!preset.trim()} className="rounded-xl border border-emerald-400/30 bg-emerald-400/10 px-4 py-2 text-xs font-semibold text-emerald-300 disabled:opacity-40">发送到当前对话</button></div></section>
