...
function SelectionChip({ checked, disabled, onChange, title, subtitle }) {
  return <label className={`flex cursor-pointer items-start gap-2 rounded-xl border px-3 py-2 text-xs transition-colors ${checked ? "border-emerald-400/40 bg-emerald-400/10" : "border-cs2-border-subtle bg-cs2-bg-input/40"} ${disabled ? "cursor-not-allowed opacity-40" : ""}`}>
    ...
    <span className="min-w-0"><b className="block break-words">{title}</b>{subtitle&&<span className="mt-0.5 block break-words text-cs2-text-muted">{subtitle}</span>}</span>
  </label>;
}

export default function LeagueAccountTools({ data, enabled, busy, onBusyChange, onRefresh, onError }) {
  ...
  return <div className="space-y-4">
    <section className={`rounded-2xl border p-4 ${enabled?"border-amber-400/30 bg-amber-400/[.06]":"border-cs2-border bg-cs2-bg-elevated"}`}>
      <div className="flex items-start gap-3"><ShieldAlert className={`mt-0.5 h-5 w-5 ${enabled?"text-amber-300":"text-cs2-text-muted"}`}/><div><h3 className="text-sm font-bold">账号写入保护</h3><p className="mt-1 text-xs leading-5 text-cs2-text-muted">此处所有功能默认关闭；启用后也不会自动执行。每次领取或删除仍需明确勾选目标并输入确认短语。不会随机选择奖励。</p></div></div>
    </section>
