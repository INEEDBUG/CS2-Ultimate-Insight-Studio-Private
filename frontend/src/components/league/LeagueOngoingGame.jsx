    ...
    <div className="grid gap-2 p-3 md:grid-cols-2">
      {players.map((player, index) => {
        ...
        const usageLine = usage.mode === "mastery"
          ? `${player.champion_name || "当前英雄"} · 熟练度 ${usage.mastery_level || 0} / ${Number(usage.mastery_points || 0).toLocaleString()} 点`
          : usage.mode === "none"
        ...
        return <button key={player.puuid || index} onClick={() => player.puuid && onOpenPlayer(player.puuid)} className={`flex items-start gap-3 rounded-xl border p-3 text-left hover:border-emerald-400/30 ${cardBorder}`}>
