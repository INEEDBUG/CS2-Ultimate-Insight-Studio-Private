...
export default function LeaguePlayerCenter({ currentPuuid = "", streamerMode = false, useAliases = false, onError }) {
  ...
  const load = async (target = currentPuuid, nextPage = 0, collect = false, serverOverride) => {
    ...
    try {
      ...
      try { setSearchHistory((await fetchLeaguePlayerSearchHistory()).players || []); } catch { /* keep the current local list */ }
  ...
  useEffect(() => { load(currentPuuid); refreshRecent(); refreshSearchHistory(); refreshFriends(); refreshServers(); }, [currentPuuid]);
  ...
  return <div className="space-y-4">
    ...
    <LeaguePlayerSummary matches={data?.matches || []}/>
