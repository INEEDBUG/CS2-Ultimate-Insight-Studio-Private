...
export default function LeaguePlayerCenter({ currentPuuid = "", streamerMode = false, useAliases = false, onError }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-cs2-text-muted"/><input value={streamerMode?"":query} disabled={streamerMode} onChange={(e)=>setQuery(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&load(query,0)} placeholder={streamerMode?"直播隐私模式已隐藏 Riot ID 搜索框":"搜索 Riot ID，例如：玩家名#标签"} className="w-full rounded-xl border border-cs2-border bg-cs2-bg-input py-2 pl-9 pr-3 text-sm disabled:opacity-60"/></div><button disabled={streamerMode} onClick={()=>load(query,0)} className="rounded-xl border border-cs2-border px-4 text-xs font-semibold disabled:opacity-40"><RefreshCw className={`inline h-4 w-4 ${busy?"animate-spin":""}`}/> 读取</button><button disabled={!data?.summoner?.puuid||busy} onClick={()=>load(data.summoner.puuid,0,true)} className="rounded-xl border border-cyan-400/30 bg-cyan-400/10 px-4 text-xs font-semibold text-cyan-200 disabled:opacity-40">收集 100 场</button><button disabled={!data?.collection_count||busy} onClick={openCollection} className="rounded-xl border border-violet-400/30 bg-violet-400/10 px-4 text-xs font-semibold text-violet-200 disabled:opacity-40">本地 {data?.collection_count||0} 场</button></div>
