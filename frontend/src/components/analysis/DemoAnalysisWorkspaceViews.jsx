  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><MetricCard icon={Zap} label="Rating Pro 2.0" value={Number(proRating.rating_pro_2 || 0).toFixed(2)} detail={`修正 KAST ${Number(proRating.adjusted_kast || 0).toFixed(0)}% · 纯保枪 ${Number(proRating.pure_lost_saves || 0)}`} tone="accent" /><MetricCard icon={Activity} label="Rating Pro 3.0" value={Number(proRating.rating_pro_3 || 0).toFixed(2)} detail={`经济 ${Number(proRating.eco_factor || 1).toFixed(2)} · Swing ${Number(proRating.round_swing || 0).toFixed(3)}`} tone="green" /><MetricCard icon={Swords} label="K / D / A" value={`${player.kills} / ${player.deaths} / ${player.assists}`} detail={`${Number(player.kd || 0).toFixed(2)} K/D`} tone="blue" /><MetricCard icon={Activity} label="ADR" value={Number(player.adr || 0).toFixed(1)} detail={`总伤害约 ${Math.round(Number(player.adr || 0) * Math.max(1, data.rounds.length))}`} /><MetricCard icon={ShieldCheck} label="KAST" value={`${Number(player.kast || 0).toFixed(0)}%`} detail={`RP 修正 ${Number(proRating.adjusted_kast || 0).toFixed(0)}% · ${player.trade_kills || 0} 次补枪`} tone="green" /><MetricCard icon={Gauge} label="爆头率" value={`${Number(player.hs_percent || 0).toFixed(0)}%`} detail={`${player.first_kills || 0} 次首杀 · ${player.awp_kills || 0} 次 AWP 击杀`} tone="violet" /></div>
